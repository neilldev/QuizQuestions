...
/**
 * Accesses the quiz question "database". This is used to test out the rest of
 * the functionality. The database is an {@link ArrayList}.
 *
 * @author DJ Spiess
 */
public class QuizQuestionArrayAccess implements QuizQuestionAccessible {

    private List<QuizQuestion> questionList;
    static private final int MAX_NUMBER_OF_QUESTIONS_PER_PAGE = 10;

    /**
     * Constructor. Loads the array with quiz questions.
     */
    public QuizQuestionArrayAccess() {
        this.loadQuestionArray();
    }

    @Override
    public QuizQuestion getQuestionByIndex(long index) {
    ...
    @Override
    public List<QuizQuestion> getQuestionList(long offset) {
        long start = offset;
        if (start < 0) {
            start = 0;
        }
    ...
    @Override
    public List<QuizQuestion> getSpecifiedQuestionList(long... id) {
    ...
    /*
        Creates quiz questions and adds them to an array.
    */
    private void loadQuestionArray() {
        questionList = new ArrayList<>();
        questionList.add((new QuizQuestionBuilder())
                .id(0)
